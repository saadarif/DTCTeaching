...
- $\bar{X}$ = sample mean,  $\mu$ = population mean (usually unknown),
 
- s = sample standard deviation ,  $\sigma$ = population standard deviation (usually unknown)

- <iframe src="index.html", frameborder=0, width=1000, height=800></iframe>
...
- Lets take sample sizes of n=10 from the previous simulation experiment (10000 rolls of a single die) take the mean of those 10 samples, repeat it 10000 times and plot a histogram of the means
```{r, echo =F}
x10 <- c()
k =10000
 for ( i in 1:k) {
...
- The CLT states with increasing sample sizes (n > 30): $\bar{X}$ = $\mu$   and,  $\sigma_{\bar{X}}$ = $\frac{\sigma}{\sqrt{n}}$
